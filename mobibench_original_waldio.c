...
#define	__MOBIBENCH_ONEGUN_DEBUG__

#ifdef	__MOBIBENCH_ONEGUN_DEBUG__
//#define	__MOBIBENCH_ONEGUN_DEBUG_TESTEND_NO-DEL_DB__
//#define	__MOBIBENCH_ONEGUN_DEBUG_PRINT__
//#define	__MOBIBENCH_ONEGUN_UPDATE_AFTER_INSERT__
//#define	__MOBIBENCH_ONEGUN_DEBUG_FORCE_INIT__
#endif

//#define	__MOBIBENCH_ONEGUN_AUTOINCREMENT__
...
float get_current_sectime(void)
{
	struct timeval current;
	
	gettimeofday(&current,NULL);

	current.tv_sec = current.tv_sec%10000;
	
	return (float)((float)current.tv_sec + (float)current.tv_usec/1000000);	
}
#endif
...
#define INSERT_STR "aaaaaaaaaabbbbbbbbbbccccccccccddddddddddeeeeeeeeeeffffffffffgggggggggghhhhhhhhhhiiiiiiiiiijjjjjjjjjj" // 10 * 10
#ifdef	__MOBIBENCH_ONEGUN_DEBUG__
#define UPDATE_STR "kkkkkkkkklllllllllmmmmmmmmmmnnnnnnnnnnooooooooooppppppppppqqqqqqqqqqrrrrrrrrrrsssssssssstttttttttt"
#else
...
int init_db_for_update(sqlite3* db, char* filename, int trs)
{
	...
	for(i = 0; i < trs; i++) 
	{
		...
		char cmd_buffer[256];
#endif
#if 0//def	__MOBIBENCH_ONEGUN_DEBUG_PRINT__
		printf("[MOBI] %s() INSERT (a~j) i=%d\n", __func__, i);
#endif
#ifdef	__MOBIBENCH_ONEGUN_DEBUG__
		sprintf(cmd_buffer, "INSERT INTO tblMyList(Value) VALUES(\'%04d_%s\');", i, INSERT_STR);
		exec_sql(db, cmd_buffer, NULL);
#else
//		exec_sql(db, "INSERT INTO tblMyList(Value) VALUES('"INSERT_STR"');", NULL);
		sprintf(buf, "INSERT INTO tblMyList(id, Value) VALUES(%d, '"INSERT_STR"');", i);
		exec_sql(db, cmd_buffer, NULL);	
#endif
...
//	printf("db thread start\n");
	...
	sprintf(filename, "%s/test.db%d-%d", pathname, thread_num, db_journal_mode);
	printf("open db : %s\n", filename);
...
//	exec_sql(db, "CREATE TABLE IF NOT EXISTS tblMyList (id INTEGER PRIMARY KEY, Value TEXT not null, creation_date long);", NULL);
	exec_sql(db, "CREATE TABLE IF NOT EXISTS tblMyList (id INTEGER PRIMARY KEY, Value TEXT not null);", NULL);
//  char buf[2048];
//  char insert_stmt[] = "insert into tblMyList (id, value) values (%d, '"INSERT_STR"')"; //128
//  sprintf(buf, insert_stmt, INT_MAX);
//  rc = sqlite3_exec(db, buf, 0, 0, 0);
	...
	if(db_mode != 0)
	{
		...
		if(column_count < db_transactions)
		{
			...
			init_db_for_update(db, filename, db_transactions);
		}
	}
	...
	{
#ifdef	__MOBIBENCH_ONEGUN_DEBUG__
		static unsigned long insert_size = 0;
		static int cnt=0;
		char cmd_buffer[256];
#endif
		if(db_mode == 0)
		{
      char buf[1000];
#ifdef	__MOBIBENCH_ONEGUN_DEBUG_PRINT__
      printf("[%f][MOBI] %s() INSERT (a~j) total size=%ld cnt=%d\n", get_current_sectime(), __func__, insert_size+=(sizeof(INSERT_STR)+4), cnt);
#endif
      //sprintf(buf, "INSERT INTO tblMyList(id, Value) VALUES(%d, '"INSERT_STR"');", i);
      sprintf(buf, "INSERT INTO tblMyList(Value) VALUES('"INSERT_STR"');");
			//exec_sql(db, "INSERT INTO tblMyList(Value) VALUES('"INSERT_STR"');", NULL);
			exec_sql(db, buf, NULL);
		}
		else if(db_mode == 1)
		{
#ifdef	__MOBIBENCH_ONEGUN_DEBUG_PRINT__
			...
			srand(i);
#ifdef	__MOBIBENCH_ONEGUN_DEBUG_PRINT__
			tmp_rand = rand()%db_transactions + 1;
			printf("[%f][MOBI] %s() UPDATE(cnt:%d) (%d)<-(k~t)\n", get_current_sectime(), __func__, i, tmp_rand);
#endif
			//sprintf(sql, "UPDATE tblMyList SET Value = '%s' WHERE id = %d;", UPDATE_STR, rand()%db_transactions + 1);
			sprintf(sql, "UPDATE tblMyList SET Value = '%s' WHERE id = %d;", UPDATE_STR, i);
			exec_sql(db, sql, NULL);
		}
		else if(db_mode == 2)
		{
#ifdef	__MOBIBENCH_ONEGUN_DEBUG_PRINT__
			printf("[%f][MOBI] %s() DELETE (%d)\n", get_current_sectime(), __func__, i);
#endif
		...
		{
			sprintf(sql, "SELECT FROM tblMyList WHERE id=%d;", rand()%db_transactions + 1);
			exec_sql(db, sql, NULL);
		}
		else
		{
			printf("[ERROR] %s() invaild db operation mode (%d)\n", __func__, i);
	...
	{
#ifdef	__MOBIBENCH_ONEGUN_DEBUG__
		printf("[%f][MOBI] %s() checkpoint wal file\n", get_current_sectime(), __func__);
#else
		printf("%s() checkpoint wal file\n", __func__);
#endif
...
int main( int argc, char **argv)
{
	...
	printf("[%f][MOBI] %s() massurement start !!\n", ((float)(T1.tv_sec%10000) + (float)T1.tv_usec/1000000), __func__);
	...
	printf("[%f][MOBI] %s() massurement end !!\n", ((float)(T2.tv_sec%10000) + (float)T2.tv_usec/1000000), __func__);
	print_time(T1, T2);
